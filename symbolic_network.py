...
class SymbolicLayer:
    ...
    def __init__(self, funcs=None, initial_weight=None, variable=True, init_stddev=0.1):
        """
...
#            print(self.W.dtype)
    ...
    def get_npweight(self):
#        v = self.W.eval()
        v = self.W.numpy()
        return v

    def set_npweight(self, newW):
        self.W.assign(newW)
#        curweight = np.array(self.get_npweight())
#        npnew = tf.constant(np.array(newW)-curweight)
#        self.W = tf.compat.v1.assign_add(self.W, npnew)

    def get_weight(self):
        return self.W
    def set_weight(self, newW):
        self.W = newW
...
class SymbolicNet:
    ...
    def __init__(self, symbolic_depth, funcs=None, initial_weights=None, initial_bias=None,
        ...
        if initial_weights is not None:
            if len(initial_weights) != self.depth+1:
                raise IndexError("The size of inital weight list passed into the symbolic network does not match the network")
                
            if all(isinstance(elem, list) for elem in funcs):
                # Seperate ist of functions for each layer
                self.symbolic_layers = [SymbolicLayer(funcs=funcs[i], initial_weight=initial_weights[i], variable=variable)
            ...
            print('Individual layer Activation functions not implemented for this seeting')
            ...
            self.output_weight = tf.Variable(tf.zeros(shape=(self.symbolic_layers[-1].n_funcs, 1)))
...
#        self.shape = tf.size(input)     # Dimensionality of the input
        h = input
        #print(input.shape)
...
#        return [self.symbolic_layers[i].W for i in range(self.depth)] + [self.output_weight]
        for layer in self.symbolic_layers:
            outlayer = layer.get_npweight()
            outlist.append(outlayer)
#        v = self.output_weight.eval()
        v = self.output_weight.numpy()
        outlist.append(v)
        return outlist
...
#        return [self.symbolic_layers[i].W for i in range(self.depth)] + [self.output_weight]
        for layer in self.symbolic_layers:
            outlayer = layer.get_weight()
            outlist.append(outlayer)
        outlist = outlist + [self.output_weight]
        return outlist

    def set_npweights(self, weightlist):
        """Sets network weight matrices"""
        if len(self.symbolic_layers)+1 != len(weightlist):
            raise TypeError("Shape of the passed weight list does not match the expected shape.")
        for layer, newweight in zip(self.symbolic_layers, weightlist[:-1]):
            layer.set_npweight(newweight)
        self.output_weight.assign(weightlist[-1])
#        v = self.output_weight.eval()
#        v = self.output_weight.numpy()
#        curweight = np.array(v)
#        npnew = tf.constant(np.array(weightlist[-1])-curweight)
#        self.output_weight = tf.compat.v1.assign_add(self.output_weight, npnew)
#        self.output_weight.assign(weightlist[-1])

    def set_weights(self, weightlist):
        """Return list of weight matrices"""
        for layer, newweight in zip(self.symbolic_layers, weightlist[:-1]):
            layer.set_weight(newweight)
        self.output_weight = weightlist[-1][0]
#            return [self.symbolic_layers[i].W for i in range(self.depth)] + [self.output_weight]

    def creategradtape(self, x_train, y_train, lossfunc):
        layers = self.get_weights()
        with tf.GradientTape() as grad:
            for weights in layers:
                grad.watch(weights)
            y = self(x_train)
            err = lossfunc(y, y_train)
        return grad, err

    def computegrad(self, x_train, y_train, lossfunc):
        grad, err = self.creategradtape(x_train, y_train, lossfunc)
        outgrad = grad.gradient(err, grad.watched_variables())
        return list(outgrad)
...
class SymbolicLayerL0(SymbolicLayer):
    ...
    def quantile_concrete(self, u):
        ...
        y = tf.math.sigmoid((tf.math.log(u) - tf.math.log(1.0-u) + self.qz_log_alpha) / BETA)
    ...
    def sample_z(self, batch_size, sample=True):
        ...
        if sample:
            ...
            pi = tf.math.sigmoid(self.qz_log_alpha)
    ...
    def loss(self):
        ...
        return tf.reduce_sum(tf.math.sigmoid(self.qz_log_alpha - BETA * tf.math.log(-GAMMA / ZETA)))
